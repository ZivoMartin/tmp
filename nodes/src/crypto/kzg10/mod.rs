...
pub(crate) use ark_std::start_timer;
pub(crate) use ark_std::end_timer;
pub(crate) use ark_poly_commit::{Error, PCRandomness};
pub(crate) use ark_ec::AffineRepr;
pub(crate) use ark_ec::{pairing::Pairing, CurveGroup};
pub(crate) use ark_ec::{scalar_mul::fixed_base::FixedBase, VariableBaseMSM};
pub(crate) use ark_ff::{One, PrimeField, UniformRand, Zero};
pub(crate) use ark_poly::DenseUVPolynomial;
pub(crate) use ark_std::{format, marker::PhantomData, ops::Div, ops::Mul, vec};

pub(crate) use ark_std::collections::BTreeMap;
...
pub struct KZG10<ArkE: Pairing, ArkP: DenseUVPolynomial<ArkE::ScalarField>> {
    _engine: PhantomData<ArkE>,
    _poly: PhantomData<ArkP>,
}

impl<ArkE, ArkP> KZG10<ArkE, ArkP>
where
    ArkE: Pairing,
    ArkP: DenseUVPolynomial<ArkE::ScalarField, Point = ArkE::ScalarField>,
    for<'a, 'b> &'a ArkP: Div<&'b ArkP, Output = ArkP>,
{
    ...
    pub fn setup<R: RngCore>(
        ...
        rng: &mut R,
    ) -> Result<UniversalParams<ArkE>, Error> {
        if max_degree < 1 {
            return Err(Error::DegreeIsZero);
        }
        let setup_time = start_timer!(|| format!("KZG10::Setup with degree {}", max_degree));
        let beta = ArkE::ScalarField::rand(rng);
        let g = ArkE::G1::rand(rng);
        let gamma_g = ArkE::G1::rand(rng);
        let h = ArkE::G2::rand(rng);

        let mut powers_of_beta = vec![ArkE::ScalarField::one()];
        ...
        let scalar_bits = ArkE::ScalarField::MODULUS_BIT_SIZE as usize;
        let g_time = start_timer!(|| "Generating powers of G");
        let g_table = FixedBase::get_window_table(scalar_bits, window_size, g);
        let powers_of_g =
            FixedBase::msm::<ArkE::G1>(scalar_bits, window_size, &g_table, &powers_of_beta);
        end_timer!(g_time);
        let gamma_g_time = start_timer!(|| "Generating powers of gamma * G");
        let gamma_g_table = FixedBase::get_window_table(scalar_bits, window_size, gamma_g);
        let mut powers_of_gamma_g =
            FixedBase::msm::<ArkE::G1>(scalar_bits, window_size, &gamma_g_table, &powers_of_beta);
        ...
        let powers_of_g = ArkE::G1::normalize_batch(&powers_of_g);
        let powers_of_gamma_g = ArkE::G1::normalize_batch(&powers_of_gamma_g)
            .into_iter()
            .enumerate()
            .collect();
        ...
        let neg_powers_of_h = if produce_g2_powers {
            let mut neg_powers_of_beta = vec![ArkE::ScalarField::one()];
            let mut cur = ArkE::ScalarField::one() / &beta;
            for _ in 0..max_degree {
            ...
            let neg_powers_of_h = FixedBase::msm::<ArkE::G2>(
                scalar_bits,
                window_size,
                &neg_h_table,
                &neg_powers_of_beta,
            ...
            let affines = ArkE::G2::normalize_batch(&neg_powers_of_h);
    ...
    pub fn commit(
    ...
    ) -> Result<(Commitment<ArkE>, Randomness<ArkE::ScalarField, ArkP>), Error> {
        ...
        let mut commitment = <ArkE::G1 as VariableBaseMSM>::msm_bigint(
            &powers.powers_of_g[num_leading_zeros..],
            &plain_coeffs,
        );
        end_timer!(msm_time);

        let mut randomness = Randomness::<ArkE::ScalarField, ArkP>::empty();
        ...
        let random_commitment = <ArkE::G1 as VariableBaseMSM>::msm_bigint(
    ...
    pub fn compute_witness_polynomial(
        p: &ArkP,
        point: ArkP::Point,
        randomness: &Randomness<ArkE::ScalarField, ArkP>,
    ) -> Result<(ArkP, Option<ArkP>), Error> {
        let divisor = ArkP::from_coefficients_vec(vec![-point, ArkE::ScalarField::one()]);
    ...
    pub(crate) fn open_with_witness_polynomial<'a>(
        ...
        point: ArkP::Point,
        randomness: &Randomness<ArkE::ScalarField, ArkP>,
        witness_polynomial: &ArkP,
        hiding_witness_polynomial: Option<&ArkP>,
    ) -> Result<Proof<ArkE>, Error> {
        Self::check_degree_is_too_large(witness_polynomial.degree(), powers.size())?;
        let (num_leading_zeros, witness_coeffs) =
            skip_leading_zeros_and_convert_to_bigints(witness_polynomial);

        let witness_comm_time = start_timer!(|| "Computing commitment to witness polynomial");
        let mut w = <ArkE::G1 as VariableBaseMSM>::msm_bigint(
            &powers.powers_of_g[num_leading_zeros..],
            &witness_coeffs,
        );
        ...
        let random_v = if let Some(hiding_witness_polynomial) = hiding_witness_polynomial {
            ...
            w += &<ArkE::G1 as VariableBaseMSM>::msm_bigint(
                &powers.powers_of_gamma_g,
    ...
    pub(crate) fn open<'a>(
        ...
        p: &ArkP,
        point: ArkP::Point,
        rand: &Randomness<ArkE::ScalarField, ArkP>,
    ) -> Result<Proof<ArkE>, Error> {
    ...
    pub fn check(
        vk: &VerifierKey<ArkE>,
        comm: &Commitment<ArkE>,
        point: ArkE::ScalarField,
        value: ArkE::ScalarField,
        proof: &Proof<ArkE>,
    ) -> Result<bool, Error> {
        ...
        let lhs = ArkE::pairing(inner, vk.h);

        let inner = vk.beta_h.into_group() - &vk.h.mul(point);
        let rhs = ArkE::pairing(proof.w, inner);
    ...
    pub fn batch_check<R: RngCore>(
        vk: &VerifierKey<ArkE>,
        c: &Commitment<ArkE>,
        points: &[ArkE::ScalarField],
        values: &[ArkE::ScalarField],
        proofs: &[Proof<ArkE>],
        rng: &mut R,
    ) -> Result<bool, Error> {
        let check_time =
        ...
        let mut total_c = <ArkE::G1>::zero();
        let mut total_w = <ArkE::G1>::zero();

        let combination_time = start_timer!(|| "Combining commitments and proofs");
        let mut randomizer = ArkE::ScalarField::one();
        // Instead of multiplying g and gamma_g in each turn, we simply accumulate
        // their coefficients and perform a final multiplication at the end.
        let mut g_multiplier = ArkE::ScalarField::zero();
        let mut gamma_g_multiplier = ArkE::ScalarField::zero();
        ...
        let affine_points = ArkE::G1::normalize_batch(&[-total_w, total_c]);
        let (total_w, total_c) = (affine_points[0], affine_points[1]);
        end_timer!(to_affine_time);

        let pairing_time = start_timer!(|| "Performing product of pairings");
        let result = ArkE::multi_pairing(
            [total_w, total_c],
    ...
    pub fn trim(
        pp: &UniversalParams<ArkE>,
        mut supported_degree: usize,
    ) -> Result<(Powers<ArkE>, VerifierKey<ArkE>), Error> {
...
fn skip_leading_zeros_and_convert_to_bigints<F: PrimeField, ArkP: DenseUVPolynomial<F>>(
    p: &ArkP,
) -> (usize, Vec<F::BigInt>) {
...
mod tests {
    ...
    fn end_to_end_test_template<ArkE, ArkP>() -> Result<(), Error>
    where
        E: Pairing,
        P: DenseUVPolynomial<ArkE::ScalarField, Point = ArkE::ScalarField>,
        for<'a, 'b> &'a P: Div<&'b P, Output = P>,
    {
        ...
        for _ in 0..100 {
            ...
            let pp = KZG10::<ArkE, ArkP>::setup(degree, false, rng)?;
            let (ck, vk) = KZG10::<ArkE, ArkP>::trim(&pp, degree)?;
            let p = ArkP::rand(degree, rng);
            let hiding_bound = Some(1);
            let (comm, rand) = KZG10::<ArkE, ArkP>::commit(&ck, &p, hiding_bound, Some(rng))?;
            let point = ArkE::ScalarField::rand(rng);
            let value = p.evaluate(&point);
            let proof = KZG10::<ArkE, ArkP>::open(&ck, &p, point, &rand)?;
            assert!(
                KZG10::<ArkE, ArkP>::check(&vk, &comm, point, value, &proof)?,
    ...
    fn linear_polynomial_test_template<ArkE, ArkP>() -> Result<(), Error>
    where
        E: Pairing,
        P: DenseUVPolynomial<ArkE::ScalarField, Point = ArkE::ScalarField>,
        for<'a, 'b> &'a P: Div<&'b P, Output = P>,
    {
        ...
        for _ in 0..100 {
            let degree = 50;
            let pp = KZG10::<ArkE, ArkP>::setup(degree, false, rng)?;
            let (ck, vk) = KZG10::<ArkE, ArkP>::trim(&pp, 2)?;
            let p = ArkP::rand(1, rng);
            let hiding_bound = Some(1);
            let (comm, rand) = KZG10::<ArkE, ArkP>::commit(&ck, &p, hiding_bound, Some(rng))?;
            let point = ArkE::ScalarField::rand(rng);
            let value = p.evaluate(&point);
    ...
    fn batch_check_test_template<ArkE, ArkP>() -> Result<(), Error>
    where
        E: Pairing,
        P: DenseUVPolynomial<ArkE::ScalarField, Point = ArkE::ScalarField>,
        for<'a, 'b> &'a P: Div<&'b P, Output = P>,
    {
        ...
        for _ in 0..10 {
            ...
            for _ in 0..10 {
                ...
                assert!(KZG10::<ArkE, ArkP>::check(&vk, &comm, point, value, &proof)?);
                comms.push(comm);
                values.push(value);
                points.push(point);
                proofs.push(proof);
            }
            assert!(KZG10::<ArkE, ArkP>::batch_check(
